...
internal sealed class OracleCIEmailProvider : IEmailProvider
{
    ...
    public OracleCIEmailProvider(IOptionsSnapshot<EmailSettings> settings) => _settings = settings;
    ...
    public async Task SendEmailAsync(string to, string subject, string htmlBody, CancellationToken ct)
    {
        ...
        using Activity? activity = DiagnosticsConfig.MailActivitySource.StartActivity("OracleCIProvider.Send");
        activity?.SetTag("mail.provider", "oracle_ci");
        ...
        finally
        {
            DiagnosticsConfig.EmailSendDuration.Record(
                ...
                new KeyValuePair<string, object?>("provider", "oracle_ci"));
