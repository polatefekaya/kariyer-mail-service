...
internal sealed class SendGridEmailProvider : IEmailProvider
{
    ...
    public async Task SendEmailAsync(string to, string subject, string htmlBody, CancellationToken ct)
    {
        ...
        using Activity? activity = DiagnosticsConfig.MailActivitySource.StartActivity("SendGridProvider.Send");
        activity?.SetTag("mail.provider", "sendgrid");
        ...
        finally
        {
            DiagnosticsConfig.EmailSendDuration.Record(
                ...
                new KeyValuePair<string, object?>("provider", "sendgrid"));
        }
    }
}
